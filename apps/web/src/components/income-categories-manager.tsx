...
import {
  ...
  setIncomeCategoryHidden,
  seedIncomeCategoriesForCompany,
  queryKeys,
...
export function IncomeCategoriesManager({ companyId, canEdit }: Props) {
  ...
  const { data: categories, isLoading } = useQuery({
    queryKey: queryKeys.incomeCategories(companyId, 'all'),
    queryFn: () => getIncomeCategories(companyId, { includeHidden: true }),
  });
  ...
  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ['incomeCategories', companyId] });
  };

  const createMutation = useMutation({
    mutationFn: () => createIncomeCategory({ name_en: newName }),
    onSuccess: () => {
      setNewName('');
      invalidate();
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, name }: { id: string; name: string }) =>
      updateIncomeCategory(id, { name_en: name }),
    onSuccess: () => {
      setEditingId(null);
      setEditName('');
      invalidate();
  ...
  const hideMutation = useMutation({
    mutationFn: ({ id, hidden }: { id: string; hidden: boolean }) =>
      setIncomeCategoryHidden(id, hidden),
    onSuccess: () => {
      invalidate();
      toast(t('success'), 'success');
    },
  ...
  const syncTemplateMutation = useMutation({
    mutationFn: () =>
      seedIncomeCategoriesForCompany(companyId, company?.business_type ?? 'other'),
    onSuccess: () => {
      invalidate();
      toast(t('templateIncomeCategoriesSynced'), 'success');
    },
    onError: (e: Error) => toast(e.message, 'error'),
  });
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500 dark:text-gray-400">{t('incomeCategoriesDesc')}</p>
      ...
      {canEdit && (
        <div className="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-800">
          <FormInput
            ...
            placeholder={t('incomeCategoryNamePlaceholder')}
            className="flex-1"
          />
      ...
      {canEdit && company?.business_type && (
        ...
        >
          {t('syncTemplateIncomeCategories')}
        </PremiumButton>
      )}
