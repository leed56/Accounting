...
import { customerSchema, type CustomerInput } from '@bizmanager/types';
import { AppShell } from '@/components/app-shell';
import { FormInput, TextAreaField } from '@/components/form-fields';
import { PremiumButton } from '@/components/premium-button';
import { useTranslation } from '@/components/language-switcher';
import { useToast } from '@/components/toast';
import { createCustomer, queryKeys } from '@bizmanager/supabase-client';
...
export default function AddCustomerPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      if (companyId) queryClient.invalidateQueries({ queryKey: queryKeys.customers(companyId) });
